...
ElastProblem::ElastProblem (const GetPot& dataFile, Bulk* bulk ):
			...
			M_DispScalarFEM(bulk->getMesh(),dataFile ( std::string("bulkData/mecc/FEMTypeDisplacement").data (), "FEM_PK(2,1)" ), 1 ),
...
{  
   ...
   std::string intMethod(dataFile ( std::string("bulkData/mecc/integrationMethod" ).data (), "IM_TRIANGLE(6)" ) );
