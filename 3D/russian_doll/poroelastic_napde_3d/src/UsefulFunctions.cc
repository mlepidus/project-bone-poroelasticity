...
//serve solo nel caso con più faglie intersecanti e al momento non funziona
...
//per soluzioni discontinue interpolo su una mesh fine e tagliata per esportare

/*void interpolateDiscontinuousField(FEM originalspace,getfem::mesh_fem& cutspace,  std::vector<scalar_type>& field, std::vector<scalar_type>& interpolated, LevelSet* levelsetPtr )
{
	std::vector<scalar_type> fieldIn(originalspace.nb_dof("base"));
	std::vector<scalar_type> fieldOut(originalspace.nb_dof("base"));
	std::vector<scalar_type> fieldIntIn(cutspace.nb_dof());
	std::vector<scalar_type> fieldIntOut(cutspace.nb_dof());
	
	
	for (size_type i=0; i< fieldIn.size(); ++i)
	{
		fieldIn[i]=field[i];
		fieldOut[i]=field[i];
	}
	
	for (size_type i=0; i<originalspace.nb_dof("extended");++i)
	{
		size_type ii=originalspace.getExt(i);
		if (originalspace.getExtSign(i)<0)
		{
			fieldIn[ii]=field[ii];
			fieldOut[ii]=field[i+originalspace.nb_dof("base")];
		}
		else
		{
			fieldOut[ii]=field[ii];
			fieldIn[ii]=field[i+originalspace.nb_dof("base")];
		}

	}

	getfem::interpolation(*(originalspace.getFEM()), cutspace, fieldIn,fieldIntIn );
	getfem::interpolation(*(originalspace.getFEM()), cutspace, fieldOut,fieldIntOut );

	for (size_type i=0; i<cutspace.nb_dof();++i)
	{
		base_node nodo(cutspace.point_of_basic_dof(i));
		if (levelsetPtr->ls_values(nodo)<0)
		{
			interpolated[i]=fieldIntIn[i];
		}
		else
		{
			interpolated[i]=fieldIntOut[i];
		}
