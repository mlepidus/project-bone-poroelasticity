...
public: 
    ...
    void setOuterBoundaryInterpolationP(const std::string& filename, int degree);

    void setOuterBoundaryInterpolationD(const std::string& filename, int degree);

    inline double evaluateOuterBoundaryConditionPressure(double z) const {
        if (M_hasOuterBoundaryInterpolationP) {
            return M_outerBoundaryInterpolationP(z);
        } else {
            throw std::runtime_error("Outer boundary interpolation function is not set for the pressure.");
        }
    }
    inline bgeot::base_node evaluateOuterBoundaryConditionDisplacement(double z) const {
        if (M_hasOuterBoundaryInterpolationD) {
            return M_outerBoundaryInterpolationD(z);
        } else {
            throw std::runtime_error("Outer boundary interpolation function is not set for the displacement.");
        }
    }
...
private:
    ...
    std::function<double(double)> M_outerBoundaryInterpolationP;
    std::function<bgeot::base_node(double)> M_outerBoundaryInterpolationD;
    
    bool M_hasOuterBoundaryInterpolationP;
    bool M_hasOuterBoundaryInterpolationD;
};
